...
import RentalHistoryTable from "../rentalHistoryTable/rentalHistoryTable";
...
const RentalHistoryList = () => {
  ...
  const [selectedRentalHistoryId, setSelectedRentalHistoryId] = useState<number | null>(null);
  ...
  const confirmDelete = (id: number) => {
    setSelectedRentalHistoryId(id);
    setShowDeleteModal(true);
  };

  const closeModal = () => {
    setShowDeleteModal(false);
    setSelectedRentalHistoryId(null);
  };

  if (rentalHistoryLoading) {
    return (
    ...
    <>
      <div className={`mt-[10px] fade-in ${isLoaded ? "active" : ""}`}>
        <div className="flex justify-between">
          <h1 className="text-2xl font-bold mb-6">Rental History List</h1>
          <Link to="/admin/add-rental-history">
      ...
      <Modal isOpen={showDeleteModal} onClose={closeModal}>
        ...
        <button
          ...
          onClick={() => {
            handleDelete(selectedRentalHistoryId || 0);
            closeModal();
          }}
        >
